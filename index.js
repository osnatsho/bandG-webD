...
app.post("/submit", (req, res) => {
  ...
  // var randomAdjNum= Math.floor(Math.random()*adj.length);
  // const randomAdj=adj[randomAdjNum];

  // var randomNounNum= Math.floor(Math.random()*noun.length);
  // const randomNoun=noun[randomNounNum];

  const randomAdj = adj[Math.floor(Math.random() * adj.length)];
  const randomNoun = noun[Math.floor(Math.random() * noun.length)];
  //2. Send the index.ejs as a response and add the adjective and noun to the res.render
  res.render("index.ejs", {randAd :randomAdj}, {randNo: randomNoun});
